...
from dmenu_hotkeys.parsers import (
    ...
    BspwmConfigParser, AwesomeConfigParser
)
...
#
# class TestAwesomeConfigParser(unittest.TestCase):
#     def setUp(self):
#         self.parser = AwesomeConfigParser()
#
...
class TestBspwmConfigParser(unittest.TestCase):
    ...
    def test_parse_hotkey_when_all_ok(self):
        line_with_multi_space = "alt  +   F4"
        line_without_tabs = "alt + F4"
        line_with_tabs = "super + alt +\tEscape"
        line_with_next_line = "\nsuper + apostrophe\n"
        self.assertEqual(self.parser.parse_hotkey(line_with_multi_space), 'alt + F4')
...
class TestI3ConfigParser(unittest.TestCase):
    ...
    def test_parse_hotkey_when_all_ok(self):
        line_with_multi_space = "bindsym   mod+a exec some"
        line_with_tabs = "bindsym \tmod+a\t\t\texec some"
        line_without_tabs = "bindsym mod+a exec some"
        self.assertEqual(self.parser.parse_hotkey(line_with_tabs), 'mod+a')
        self.assertEqual(self.parser.parse_hotkey(line_with_tabs), 'mod+a')
